...
# Bundled Mukta covers both Latin and Devanagari in one Unicode TTF, so the whole
# report — English chrome and Hindi data/titles alike — renders from a single
# font and Devanagari no longer falls back to "?". Without it we degrade to the
# latin-1 core font (English only).
_FONT_REG = Path(__file__).parent / "assets" / "Mukta-Regular.ttf"
_FONT_BOLD = Path(__file__).parent / "assets" / "Mukta-Bold.ttf"
_NOTO_AVAILABLE = _FONT_REG.exists() and _FONT_BOLD.exists()
FONT = "Mukta" if _NOTO_AVAILABLE else "Helvetica"
...
def _latin1(text: str) -> str:
    # With the bundled Unicode font, pass text through unchanged so Hindi renders.
    # Without it, fpdf2's core fonts are latin-1 only, so drop characters they
    # can't encode rather than crashing on an export.
    if _NOTO_AVAILABLE:
        return str(text)
...
def _truncate(text: str, width_mm: float, pdf) -> str:
    """Trim text (with an ellipsis) to fit within width_mm at the current font."""
...
def _build_pdf(table: list[dict], meta: dict):
    ...
    if _NOTO_AVAILABLE:
        pdf.add_font(FONT, "", str(_FONT_REG))
        pdf.add_font(FONT, "B", str(_FONT_BOLD))
...
def _info_card(pdf, meta: dict, table: list[dict], fieldnames: list[str]) -> None:
    ...
    for i, (label, value) in enumerate(rows):
        ...
        value_w = col_w - label_w - 6
        pdf.cell(value_w, line_h, _truncate(_latin1(str(value)), value_w, pdf), align="L")
...
def _empty_notice(pdf) -> None:
    # Regular (not italic): the bundled Noto family ships only regular + bold.
    pdf.ln(20)
